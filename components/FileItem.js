...
const FileItem = ({ file }) => {
  ...
  const { removeFile, cancelUpload, updateFileStatus, uploadProgress } =
    useUploader();
  ...
  const progress = useMemo(() => {
    ...
    const progressValue =
      uploadProgress && file?.id ? uploadProgress[file.id] : null;
    if (typeof progressValue === "number" && !isNaN(progressValue)) {
      return progressValue;
    }
    // Fallback to file's internal progress
    return typeof file.progress === "number" && !isNaN(file.progress)
      ? file.progress
      : 0;
  }, [file, uploadProgress]);
    ...
    <div className="flex items-center gap-4 p-3 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
      ...
      <div className="flex-grow min-w-0">
        ...
        {file.status === FILE_STATUS.FAILED && file.error && (
          <div className="mt-1 text-xs text-error">Error: {file.error}</div>
        )}
      </div>
      ...
      <div className="flex-shrink-0 flex space-x-2">
        ...
        <button
          ...
          title={
            file.status === FILE_STATUS.UPLOADING ? "Cancel upload" : "Remove"
          }
        >
          <svg
            className="w-5 h-5"
