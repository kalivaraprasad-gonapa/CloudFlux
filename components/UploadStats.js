...
const UploadStats = () => {
  ...
  const handleReset = () => {
    if (window.confirm('Are you sure you want to reset all upload statistics?')) {
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center">
          <h2 className="text-lg font-medium text-gray-900 dark:text-white">Upload Statistics</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-secondary dark:bg-gray-700 rounded-lg p-4">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xl font-semibold text-gray-900 dark:text-white">{stats.totalUploaded}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-secondary dark:bg-gray-700 rounded-lg p-4">
          <div className="flex items-center">
            ...
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">Total Size Uploaded</p>
              <p className="text-xl font-semibold text-gray-900 dark:text-white">{formatBytes(stats.totalSize)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-secondary dark:bg-gray-700 rounded-lg p-4">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-base font-semibold text-gray-900 dark:text-white">{formatDate(stats.lastUploadDate)}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
        <div className="bg-secondary dark:bg-gray-700 rounded-lg p-4">
          <div className="flex items-center">
            ...
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">Successful Uploads</p>
              <p className="text-xl font-semibold text-gray-900 dark:text-white">{stats.successCount}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-secondary dark:bg-gray-700 rounded-lg p-4">
          <div className="flex items-center">
            <div className="rounded-md bg-error/20 p-2 mr-4">
              <svg className="w-6 h-6 text-error" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              </svg>
            </div>
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">Failed Uploads</p>
              <p className="text-xl font-semibold text-gray-900 dark:text-white">{stats.failedCount}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="mt-4">
          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
            <div 
              ...
              style={{ 
                width: `${stats.totalUploaded > 0 ? (stats.successCount / stats.totalUploaded) * 100 : 0}%` 
              }}
            ></div>
          </div>
          <div className="flex justify-between text-xs text-gray-500 dark:text-gray-400 mt-1">
            <span>Success Rate</span>
            <span>{stats.totalUploaded > 0 ? Math.round((stats.successCount / stats.totalUploaded) * 100) : 0}%</span>
          </div>
        </div>
      )}
    </div>
