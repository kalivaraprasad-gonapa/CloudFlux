...
db.version(1).stores({
  ...
  uploadQueue:
    "++id, fileName, fileSize, fileType, addedAt, status, retryCount, fileId",
});
...
export const uploadQueueService = {
  ...
  updateStatus: async (fileId, status, retryCount = null) => {
    try {
      ...
      await db.uploadQueue.where({ fileId }).modify(updates);
  ...
  removeFromQueue: async (fileId) => {
    try {
      await db.uploadQueue.where({ fileId }).delete();
  ...
  clearCompleted: async () => {
    try {
      await db.uploadQueue.where("status").equals("completed").delete();
